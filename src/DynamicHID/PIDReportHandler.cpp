...
{
    memcpy((void*)&g_EffectStates[0], &effect, sizeof(TEffectState));
    const uint8_t id = GetNextFreeEffect();
    memcpy((void*)&g_EffectStates[id], (void*)&g_EffectStates[0], sizeof(TEffectState));
    if (id != 1)
    {
...
void PIDReportHandler::PrintEffect(uint8_t id)
{
    ...
    DEBUG_PRINTLN(g_EffectStates[id].direction[0]);
#if (FFB_AXIS_COUNT>1)
    DEBUG_PRINT("  directionY ");
    DEBUG_PRINTLN(g_EffectStates[id].direction[1]);
#endif
#if (FFB_AXIS_COUNT>2)
    DEBUG_PRINT("  directionZ ");
    DEBUG_PRINTLN(g_EffectStates[id].direction[2]);
#endif
    DEBUG_PRINT("  phase ");
    ...
    DEBUG_PRINTLN((uint32_t)g_EffectStates[id].startTime);
...
void PIDReportHandler::FreeAllEffects(void)
{
    ...
    {
        memset((void*)&g_EffectStates[i], 0, sizeof(TEffectState));
    }
    if (g_EffectStates[0].state != MEFFECTSTATE_FREE)
    {
        // Default effect is enabled.
        memcpy((void*)&g_EffectStates[1], (void*)&g_EffectStates[0], sizeof(TEffectState));
        nextEID += 1;
        StartEffect(1);
    }
...
void PIDReportHandler::SetEffect(USB_FFBReport_SetEffect_Output_Data_t* data)
{
    ...
    DEBUG_PRINT("dX: ");
    DEBUG_PRINT(effect->direction[0]);
#if (FFB_AXIS_COUNT>1)
    DEBUG_PRINT(" dY: ");
    DEBUG_PRINT(effect->direction[1]);
#endif
#if (FFB_AXIS_COUNT>2)
    DEBUG_PRINT(" dZ: ");
    DEBUG_PRINT(effect->direction[2]);
#endif
    DEBUG_PRINT(" eT: ");
